...
//   try {
//     // const response = await axios.get('https://s3.amazonaws.com/roxiler.com/product_transactions.json');
//     // const transactions = response.data;
      
//     // await Transaction.deleteMany({});
//     let res1= await Transaction.insertMany(data);
//        return res.status(200).json({message:'Database initialized with seed data.', res1});
...
router.get('/data', async (req, res) => {
    ...
    if (month) {
      searchQuery.dateOfSale = { $regex: `-${month.padStart(2, '0')}-` };
    }
    ...
    const totalTransactions = await Transaction.countDocuments(search ? searchQuery : {});
